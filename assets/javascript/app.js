...
$(document).ready(selectTheme)
    ...
    function gamePlay(){
        ...
        $stage.removeClass('solidDisplay');
        defineVariables();
        backstretch();
        displayTitle();
        themeSong();
    ...
    function themeSong(){
        ...
        $themeSong = $('<audio>').attr({
            ...
            'style':'display:none','loop':true}).prop(
                'volume', 0.1).appendTo(
                    $('body')).trigger(
                        'play');
            }
    ...
    function clickThemeOrLevel(callback){
        $popUp.on('click','.difficulty',function(){
            ...
            assignLevel(Selected);
            $popUp.off().remove();
            callback();
            })
        }

    function assignLevel(selected){
        for (key in gameObj){
            if (selected == gameObj[key].name){
                themeObject = gameObj[key]
                console.log(themeObject.name)
                return;
                }
            }
    ...
    function playAgain(){
        ...
        $display.on('click','.changeTheme', function(){
